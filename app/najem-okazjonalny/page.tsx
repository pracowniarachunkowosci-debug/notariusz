...
export const metadata: Metadata = {
  ...
  alternates: { canonical: absoluteSiteUrl("/najem-okazjonalny/") },
  openGraph: { title: "Najem okazjonalny u notariusza | Wrocław", description: "Oświadczenie najemcy, dokumenty, inny lokal, 14 dni i dalsze kroki przy najmie okazjonalnym.", type: "article", url: absoluteSiteUrl("/najem-okazjonalny/") },
};

function OccasionalLeaseStructuredData() {
  const breadcrumb = occasionalLeaseData.breadcrumb.map((item, index) => ({ "@type": "ListItem", position: index + 1, name: item.label, item: absoluteSiteUrl(item.href) }));
