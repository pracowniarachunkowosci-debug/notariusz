...
export const metadata: Metadata = {
  ...
  alternates: { canonical: absoluteSiteUrl("/spadki/akt-poswiadczenia-dziedziczenia/") },
  openGraph: { title: "Akt poświadczenia dziedziczenia u notariusza | Wrocław", description: "Dokumenty, spadkobiercy, testament i kolejne kroki przy akcie poświadczenia dziedziczenia u notariusza we Wrocławiu.", type: "article", url: absoluteSiteUrl("/spadki/akt-poswiadczenia-dziedziczenia/") },
};

function InheritanceStructuredData() {
  const breadcrumbs = inheritanceCertificateData.breadcrumb.map((item, index) => ({ "@type": "ListItem", position: index + 1, name: item.label, item: absoluteSiteUrl(item.href) }));
